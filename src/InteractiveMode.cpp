...
void InteractiveMode::draw() {
    ofPushStyle();
    font.drawString("出発駅名", xRow[0], 50);
    font.drawString("到着駅名", xRow[1], 50);
    font.drawString("日付", xRow[2], 50);
    font.drawString("残高", xRow[3], 50);
    ...
    if (isVisible[isVisible.size() - 1][3]) {
        title.drawString("Where will you go next?\nPress 1 key", xRow[4], 500);
    } else {
        title.drawString("Now Loading", xRow[4] + 30, 500);
        ofDrawCircle(loadingAnim, 510, 5);
    }
    ofPopStyle();
}
